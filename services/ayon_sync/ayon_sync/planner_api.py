...
def _get_endpoint(path: str | None = None) -> str:
    addon_version = ayon_api.get_service_addon_version()
    if not path:
        return f"addons/planner/{addon_version}"
    return f"addons/planner/{addon_version}/{path}"
...
def get_resources(
    ...
    response = ayon_api.post(
        _get_endpoint("booking/resources/find"),
        **body
    )
...
def get_booking_events(
    ...
    response = ayon_api.post(
        _get_endpoint("booking/events/find"),
        **body
    )
    response.raise_for_status()
...
def update_booking_event(
    ...
    *,
    ...
    response = ayon_api.patch(
        _get_endpoint(f"booking/events/{event_id}"),
        **update_data
    )
    response.raise_for_status()


def delete_booking_event(event_id: str) -> None:
...
def get_booking_attributes() -> list[dict]:
    response = ayon_api.get(
        _get_endpoint("booking/attributes"),
    )
    response.raise_for_status()
